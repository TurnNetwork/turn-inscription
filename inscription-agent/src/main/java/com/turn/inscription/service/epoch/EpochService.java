...
import com.turn.inscription.dao.custommapper.CustomBlockNodeMapper;
            ...
            if (!nodes.isEmpty()) {
                ...
                if (prevBlockNumber.longValue() / chainConfig.getConsensusPeriodBlockCount().longValue() >= max) {
                    ...
                    nodes.forEach(node -> {
                        ...
                        com.turn.inscription.dao.entity.Node nodeT = nodeMapper.selectByPrimaryKey(node.getNodeId());
                        if (nodeT == null) {
                            blockNode.setNodeName("");
                        } else {
