...
import com.turn.inscription.enums.RedisKeyEnum;
...
public class RedisImportService {
    ...
    public void batchImport(Set<Block> blocks, Set<Transaction> transactions, Set<ErcTx> erc20TxList, Set<ErcTx> erc721TxList, Set<ErcTx> erc1155TxList) throws Exception {
        log.debug("Redis batch import: {}(blocks({}),transactions({})", Thread.currentThread().getStackTrace()[1].getMethodName(), blocks.size(), transactions. size());
        long startTime = System.currentTimeMillis();
        try {
        ...
        try {
            if (redisKeyEnum.compareTo(RedisKeyEnum.Block) == 0) {
                if (CollUtil.isNotEmpty(data)) {
                    ...
                    log.info("redis batch storage success statistics: block [{}]-[{}]", blockSum.getMin(), blockSum.getMax());
                } else {
                    log.info("redis batch storage success statistics: block [{}]-[{}]", 0, 0);
                }
            } else if (redisKeyEnum.compareTo(RedisKeyEnum.Transaction) == 0) {

            } else if (redisKeyEnum.compareTo(RedisKeyEnum.Erc20Tx) == 0) {
                log.info("redis batch storage success statistics: erc20 transaction number[{}]", data.size());
            } else if (redisKeyEnum.compareTo(RedisKeyEnum.Erc721Tx) == 0) {
                log.info("redis batch storage success statistics: erc721 transaction number [{}]", data.size());
            } else if (redisKeyEnum.compareTo(RedisKeyEnum.Erc1155Tx) == 0) {
                log.info("redis batch storage success statistics: erc1155 transaction number[{}]", data.size());
            }
        } catch (Exception e) {
            log.error("redis batch warehousing statistics exception", e);
        }
    }
