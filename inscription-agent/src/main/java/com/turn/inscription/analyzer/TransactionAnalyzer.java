package com.turn.inscription.analyzer;
...
import com.turn.inscription.decoder.TxInputDecodeResult;
import com.turn.inscription.decoder.TxInputDecodeUtil;
...
import com.turn.inscription.utils.AddressUtil;
import com.turn.inscription.utils.TransactionUtil;
import com.turn.inscription.v0152.analyzer.*;
...
public class TransactionAnalyzer {
    ...
    @Resource
    private InscriptionContractAnalyzer inscriptionContractAnalyzer;
    ...
    public CollectionTransaction analyze(Block collectionBlock, Transaction rawTransaction, Receipt receipt) throws
        ...
        if (CollUtil.isNotEmpty(receipt.getContractCreated())) {
            receipt.getContractCreated().forEach(contract -> {
                ...
                if(ErcTypeEnum.UNKNOWN.equals(ercToken.getTypeEnum())){
                    InscriptionVO inscriptionVO = inscriptionContractAnalyzer.resolveInscriptionContract(contract.getAddress());
                    if (inscriptionVO.getTypeEnum() == InscriptionTypeEnum.INSCRIPTION && txInputDecodeResult.getTypeEnum() == com.turn.inscription.elasticsearch.dto.Transaction.TypeEnum.EVM_CONTRACT_CREATE) {
                        GENERAL_CONTRACT_ADDRESS_2_TYPE_MAP.put(contract.getAddress(), ContractTypeEnum.INSCRIPTION_EVM);
                    }
                }

                // Update address type in memory
                ContractTypeEnum contractTypeEnum;
                if (ercToken.getTypeEnum() == ErcTypeEnum.ERC20 && txInputDecodeResult.getTypeEnum() == com.turn.inscription.elasticsearch.dto.Transaction.TypeEnum.EVM_CONTRACT_CREATE) {
                    contractTypeEnum = ContractTypeEnum.ERC20_EVM;
                } else if (ercToken.getTypeEnum() == ErcTypeEnum.ERC721 && txInputDecodeResult.getTypeEnum() == com.turn.inscription.elasticsearch.dto.Transaction.TypeEnum.EVM_CONTRACT_CREATE) {
                    contractTypeEnum = ContractTypeEnum.ERC721_EVM;
                } else if (ercToken.getTypeEnum() == ErcTypeEnum.ERC1155 && txInputDecodeResult.getTypeEnum() == com.turn.inscription.elasticsearch.dto.Transaction.TypeEnum.EVM_CONTRACT_CREATE) {
                    contractTypeEnum = ContractTypeEnum.ERC1155_EVM;
                } else if (txInputDecodeResult.getTypeEnum() == com.turn.inscription.elasticsearch.dto.Transaction.TypeEnum.WASM_CONTRACT_CREATE) {
        ...
        if (InnerContractAddrEnum.getAddresses()
            ...
            if (StringUtils.isBlank(result.getTo())) {
                ...
                if (GENERAL_CONTRACT_ADDRESS_2_TYPE_MAP.containsKey(result.getTo()) && inputWithoutPrefix.length() >= 8) {
                    ...
                    if (result.getStatus() == com.turn.inscription.elasticsearch.dto.Transaction.StatusEnum.SUCCESS.getCode()) {
                        // The ordinary contract call is successful and the successful proxy PPOS virtual transaction list is obtained.
                        List<com.turn.inscription.elasticsearch.dto.Transaction> successVirtualTransactions = TransactionUtil.processVirtualTx(
                                collectionBlock,
                                specialApi,
                                turnClient,
                                result,
                        ...
                        // Process inscription contracts
                        TransactionUtil.handleInscriptionContract(result,
                                receipt,
        ...
        if (InnerContractAddrEnum.getAddresses()
                ...
                && ci.getType() != com.turn.inscription.elasticsearch.dto.Transaction.TypeEnum.STAKING_TOKEN.getCode()
                && ci.getType() != com.turn.inscription.elasticsearch.dto.Transaction.TypeEnum.TRANSFER.getCode()) {
        ...
        if(ci.getContractType() != null && ContractTypeEnum.INSCRIPTION_EVM.getCode() == ci.getContractType()){
            inscriptionContractAnalyzer.resolveTx(result);
        }
