package com.turn.inscription.utils;
...
import com.turn.inscription.contract.TexasHoldem;
import com.turn.inscription.decoder.PPOSTxDecodeResult;
import com.turn.inscription.decoder.PPOSTxDecodeUtil;
import com.turn.inscription.elasticsearch.dto.Block;
import com.turn.inscription.elasticsearch.dto.Transaction;
import com.turn.inscription.enums.ContractDescEnum;
import com.turn.inscription.enums.ContractTypeEnum;
...
import com.turn.inscription.param.DelegateExitParam;
import com.turn.inscription.param.DelegateRewardClaimParam;
...
public class TransactionUtil {
    ...
    public static void resolveGeneralContractCreateTxComplementInfo(CollectionTransaction result,
        ...
        if (contractTypeEnum == ContractTypeEnum.ERC20_EVM) {
            ci.setType(com.turn.inscription.elasticsearch.dto.Transaction.TypeEnum.ERC20_CONTRACT_CREATE.getCode());
            ci.setToType(com.turn.inscription.elasticsearch.dto.Transaction.ToTypeEnum.ERC20_CONTRACT.getCode());
        } else if (contractTypeEnum == ContractTypeEnum.ERC721_EVM) {
            ci.setType(com.turn.inscription.elasticsearch.dto.Transaction.TypeEnum.ERC721_CONTRACT_CREATE.getCode());
            ci.setToType(com.turn.inscription.elasticsearch.dto.Transaction.ToTypeEnum.ERC721_CONTRACT.getCode());
        } else if (contractTypeEnum == ContractTypeEnum.ERC1155_EVM) {
            ci.setType(com.turn.inscription.elasticsearch.dto.Transaction.TypeEnum.ERC1155_CONTRACT_CREATE.getCode());
            ci.setToType(com.turn.inscription.elasticsearch.dto.Transaction.ToTypeEnum.ERC1155_CONTRACT.getCode());
        } else if (contractTypeEnum == ContractTypeEnum.WASM) {
            ci.setType(com.turn.inscription.elasticsearch.dto.Transaction.TypeEnum.WASM_CONTRACT_CREATE.getCode());
            ci.setToType(com.turn.inscription.elasticsearch.dto.Transaction.ToTypeEnum.WASM_CONTRACT.getCode());
        } else {
            ci.setType(com.turn.inscription.elasticsearch.dto.Transaction.TypeEnum.EVM_CONTRACT_CREATE.getCode());
            ci.setToType(com.turn.inscription.elasticsearch.dto.Transaction.ToTypeEnum.EVM_CONTRACT.getCode());
        }
    ...
    public static void resolveGeneralContractInvokeTxComplementInfo(CollectionTransaction tx,
        ...
        if (contractTypeEnum == ContractTypeEnum.INSCRIPTION_EVM) {
            ci.setToType(Transaction.ToTypeEnum.INSCRIPTION_CONTRACT.getCode());
            ci.setType(Transaction.TypeEnum.INSCRIPTION_CONTRACT_EXEC.getCode());
        }
    ...
    public static void handleInscriptionContract(CollectionTransaction contractInvokeTx,
                                          Receipt receipt,
                                          Web3j web3j,
                                          Logger logger, ComplementInfo ci) {
        if (CollUtil.isNotEmpty(receipt.getLogs())) {
            for (Log log : receipt.getLogs()) {
                if (CollUtil.isNotEmpty(log.getTopics())) {
                    log.getTopics().stream().forEach(topics->{
                        if (InscriptionContractCache.cache.containsKey(topics)) {
                            try {
                                InscriptionContract inscriptionContract = InscriptionContract.load(contractInvokeTx.getTo(),
                                        web3j,
                                        TexasHoldemCache.readCredentials,
                                        new DefaultGasProvider());
                                BubbleGetTransactionReceipt bubbleGetTransactionReceipt = web3j.bubbleGetTransactionReceipt(
                                ...
                                TransactionReceipt transactionReceipt = bubbleGetTransactionReceipt.getTransactionReceipt()
                                        .get();
                                List<Object> events = ReflectUtil.invoke(inscriptionContract,
                                        InscriptionContractCache.cache.get(topics),
                                        transactionReceipt);

                                HashMap<String,String> eventMap = new HashMap<>();
                                eventMap.put(InscriptionContractCache.cache.get(topics),JSONUtil.toJsonStr(events));

                                List<String> inscriptionContractEventInfo = contractInvokeTx.getInscriptionContractEventInfo();
                                if(CollUtil.isEmpty(inscriptionContractEventInfo)){
                                    inscriptionContractEventInfo = new ArrayList<>();
                                }

                                inscriptionContractEventInfo.add(JSONUtil.toJsonStr(eventMap));
                                contractInvokeTx.setInscriptionContractEventInfo(inscriptionContractEventInfo);
                                ci.setContractType(ContractTypeEnum.INSCRIPTION_EVM.getCode());
                            } catch (Exception e) {
                                logger.error(StrUtil.format("Analyzing inscription contract transaction exceptions{}", receipt.getTransactionHash()), e);
                            }
                        }});
