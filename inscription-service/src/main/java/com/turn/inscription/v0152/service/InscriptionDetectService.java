...
import com.bubble.tuples.generated.Tuple7;
...
import com.turn.inscription.v0152.bean.ErcContractId;
import com.turn.inscription.v0152.bean.GameContractId;
import com.turn.inscription.v0152.bean.InscriptionContractId;
import com.turn.inscription.v0152.contract.Erc1155Contract;
import com.turn.inscription.v0152.contract.Erc20Contract;
import com.turn.inscription.v0152.contract.Erc721Contract;
import com.turn.inscription.v0152.contract.ErcContract;
...
@Slf4j
@Service
public class InscriptionDetectService {
    ...
    @Retryable(value = BubbleCallTimeoutException.class, maxAttempts = CommonConstant.reTryNum)
    public InscriptionContractId getContractId(String contractAddress) throws BubbleCallTimeoutException {
        InscriptionContractId inscriptionContractId = null;
        try {
            inscriptionContractId = isInscriptionContract(contractAddress);

            if (StrUtil.isNotBlank(inscriptionContractId.getTick())) {
                // Get game contract information
                log.info("This contract [{}] is a inscription contract", contractAddress);
                return inscriptionContractId;
            }
    ...
    private InscriptionContractId isInscriptionContract(String contractAddress) throws
            BubbleCallTimeoutException {
        InscriptionContract inscriptionContract = InscriptionContract.load(contractAddress,
                turnClient.getWeb3jWrapper().getWeb3j(),
                CREDENTIALS,
                GAS_PROVIDER);
        return resolveContractIdInscriptionContract(inscriptionContract);
    }

    private InscriptionContractId resolveContractIdInscriptionContract(InscriptionContract inscriptionContract) throws BubbleCallTimeoutException {
        InscriptionContractId inscriptionContractId = new InscriptionContractId();
        try {
            Tuple9<String, BigInteger, String, byte[], BigInteger, BigInteger, BigInteger, BigInteger, BigInteger> inscriptionInfo = inscriptionContract.getInScriptionInfo().send();
            inscriptionContractId.setDeployBy(inscriptionInfo.getValue1());
            inscriptionContractId.setDeployTime(inscriptionInfo.getValue2().longValue());
            inscriptionContractId.setTick(inscriptionInfo.getValue3());
            inscriptionContractId.setInscriptionId(Numeric.toHexString(inscriptionInfo.getValue4()));
            inscriptionContractId.setTotalSupply(inscriptionInfo.getValue5().longValue());
            inscriptionContractId.setLimitPerMint(inscriptionInfo.getValue6().longValue());
            inscriptionContractId.setMaxPerWallet(inscriptionInfo.getValue7().longValue());
            inscriptionContractId.setTypeEnum(InscriptionTypeEnum.INSCRIPTION);
        } catch (BubbleCallTimeoutException e) {
            throw e;
        } catch (ContractCallException e) {
            log.error(" not inscription contract,{}", inscriptionContract, e);
        } catch (Exception e) {
